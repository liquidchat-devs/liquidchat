module.exports = {
    addServer(db, server) {
        ...
        db.sqlConn.promise().execute(query, [ server.id, server.name.replace("'", "\\'"), server.createdAt,  server.author.id, server.avatar, server.channels.join(","), server.members.join(",") ])
        .then((result, err) => {
            if(err) { throw err; }
        });
    },
    ...
    addUser(db, user) {
        ...
        db.sqlConn.promise().execute(query, [ user.id, user.username.replace("'", "\\'"), user.createdAt, user.avatar, user.password, user.friends.join(","), user.dmChannels.join(","), user.servers.join(","), user.status ])
        .then((result, err) => {
            if(err) { throw err; }
        });
    },
    ...
    addChannel(db, channel) {
        ...
        var query1 = [ channel.id, channel.name.replace("'", "\\'"), channel.type, channel.createdAt, channel.author.id ]
