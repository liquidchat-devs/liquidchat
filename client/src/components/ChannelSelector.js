...
export default class ChannelSelector extends React.Component {
  ...
  render() {
    ...
    let selectedServer = this.props.state.selectedServer == -1 ? undefined : this.props.functions.getServer(this.props.state.selectedServer);
    ...
    const friendRequestsList = friendRequests.map((friendRequest, i) => {
      const author = this.props.functions.getUser(friendRequest.author.id)
      const target = this.props.functions.getUser(friendRequest.target.id)
      ...
      return (
        <div key={i} className="friendRequestEntry selectedColor" style={{ height: author.id === this.props.state.session.userID ? 117 : 81}}>
          <div className="flex">
            <img alt="" className="avatar marginleft2 margintop1" src={this.props.state.fileEndpoint + "/" + user.avatar} onContextMenu={(e) => { this.props.functions.setSelectedUser(user.id); this.props.functions.setBox(e.pageX, e.pageY); this.props.functions.switchDialogState(6); e.preventDefault(); e.stopPropagation(); } }/>
          ...
          <div>
            <div className="flex">
              <div className="white channel2 pendingColor">
                Pending
              </div>
            </div>
            <div className="flex">
            <div className="white channel2 declineColor" onClick={() => { this.declineFriendRequest(friendRequest.id); }}>
                Cancel
              </div>
            </div>
          </div>
          :
          <div className="flex">
            <div className="white channel2 acceptColor" onClick={() => { this.acceptFriendRequest(friendRequest.id); }}>
              Accept
            </div>
            <div className="white channel2 declineColor" onClick={() => { this.declineFriendRequest(friendRequest.id); }}>
              Decline
    ...
    const friendList = loggedUser.friends.map((friendID, i) => {
      ...
      return (
        <div key={i} className="friendEntry selectedColor" onContextMenu={(e) => { this.props.functions.setSelectedUser(friend.id); this.props.functions.setBox(e.pageX, e.pageY); this.props.functions.switchDialogState(6); e.preventDefault(); e.stopPropagation(); } }>
    ...
    const serverList = Array.from(this.props.functions.getOwnServers().values()).map((server, i) => {
      let serverName = server.name.length < 12 ? server.name : server.name.substring(0, 9) + "..."
      return (
        <div key={i}>
          <div className={this.props.state.selectedServer === server.id ? "white headerColor server selectedColor" : "white headerColor server"} onClick={() => { this.props.functions.setSelectedServer(server.id); }} onContextMenu={(e) => { this.props.functions.setSelectedServer(server.id); this.props.functions.setBox(e.pageX, e.pageY); this.props.functions.switchDialogState(17); e.preventDefault(); e.stopPropagation(); } }>
            <div className="itemOverlay"></div>
            <div className="itemData">
    ...
    let voiceGroupChannel = voiceGroup !== -1 ? this.props.functions.getChannel(voiceGroup.id) : undefined;
    let voiceGroupServer = voiceGroupChannel !== undefined ? this.props.functions.getServer(voiceGroupChannel.server.id) : undefined;
    let channelWidth = "";
    ...
    return (
      <div className="flex">
        <div className="servers headerColor" style={{ height: this.props.state.pageHeight - 83 - this.props.state.pageHeightOffset }}>
          <div className={this.props.state.channelTypes === 3 ? "white headerColor server2 selectedColor" : "white headerColor server2"} onClick={() => { this.props.functions.switchChannelTypes(3) }}>
            Friends
          </div>
          <div className={this.props.state.channelTypes === 1 ? "white headerColor server2 selectedColor" : "white headerColor server2"} onClick={() => { this.props.functions.switchChannelTypes(1) }}>
            DMs
          </div>
          {serverList}
          <div className="white errorColor server2" onClick={() => { this.props.functions.switchDialogState(16) }}>
            + Server
          </div>
          {false ? <div className="white errorColor server2" onClick={() => { this.props.functions.switchDialogState(24) }}>
            Discovery
          </div> : ""}
        </div>
        ...
        <div>
          <div className={(this.props.state.channelTypes === 2 ? "channels" : "dmChannels") + " headerColor"} style={{ height: this.props.state.pageHeight - 83 - this.props.state.pageHeightOffset - (voiceGroup !== -1 ? 83 : 0) }}>
            ...
            onChange={({ oldIndex, newIndex }) =>
              this.props.functions.moveChannel(channels, oldIndex, newIndex)
            }
            values={channels}
            beforeDrag={({ index }) =>
              this.props.functions.switchChannel(channels[index].id)
            }
                ...
                case 1:
                  ...
                  if(voiceGroup !== -1 && voiceGroupChannel !== undefined && voiceGroupChannel.id === value.id) {
                    userList = voiceGroup.users.map((userID, i) => {
                      const user = this.props.functions.getUser(userID)
          
                      return (
                        <div key={i} className="voiceUserEntry aligny">
                          <img alt="" className="avatar marginleft1" src={this.props.state.fileEndpoint + "/" + user.avatar} onContextMenu={(e) => { this.props.functions.setSelectedUser(user.id); this.props.functions.setBox(e.pageX, e.pageY); this.props.functions.switchDialogState(6); e.preventDefault(); e.stopPropagation(); } }/>
                  ...
                  return (
                    <div key={index}>
                      <div {...props} className={"white headerColor channel"} style={{ backgroundColor: (voiceGroup !== -1 && voiceGroupChannel.id === value.id ? "#67b167" : "var(--color4)") }} onContextMenu={(e) => { this.props.functions.setSelectedChannel(value.id); this.props.functions.setBox(e.pageX, e.pageY); this.props.functions.switchDialogState(10); e.preventDefault(); e.stopPropagation(); } }>
                        <div className="itemOverlay"></div>
                        <div className="itemData">
            ...
            {this.props.state.channelTypes === 2 ?
              <div className="white headerColor channel" onClick={() => { this.props.functions.switchDialogState(1) }}>
        ...
        <div className="friends headerColor" style={{ height: this.props.state.pageHeight - 83 - this.props.state.pageHeightOffset }}>
          {friendList}
          {friendRequestsList}
          <div className="white headerColor friend" onClick={() => { this.props.functions.switchDialogState(7) }}>
            Add Friend
          </div>
        </div>}
