...
class API {
    ...
    async API_sendDM(userID, message) {
        var suitableChannels = this.mainClass.state.channels.filter(channel => { return channel.members !== undefined && channel.members.length === 2 && channel.members.includes(this.mainClass.state.session.userID) && channel.members.includes(userID); });
        var channel = -1;
        if(suitableChannels.length < 1) {
            ...
            if(isNaN(channel) === false) { return channel; }
        } else {
            channel = suitableChannels[0].id;
        }
